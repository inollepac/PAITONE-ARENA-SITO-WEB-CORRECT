...
const Hero: React.FC<HeroProps> = ({ config, isEditMode, onUpdateConfig, onBookingClick }) => {
  ...
  return (
    <section className="relative h-[90vh] flex items-center overflow-hidden bg-brand-blue">
      ...
      {/* Content Layer */}
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-white w-full text-center">
          ...
          <div className="w-full flex flex-col items-center">
            {config.heroLogo.enabled && (
              <motion.div 
                ...
                className="mb-12 overflow-hidden flex items-center justify-center bg-white/10 backdrop-blur-md"
                style={{ 
            ...
            {isEditMode ? (
              <div className="space-y-6 w-full max-w-4xl">
                <textarea 
                  ...
                  className="w-full bg-white/10 border-none rounded-3xl p-6 text-6xl md:text-8xl font-black uppercase italic tracking-tighter text-white focus:outline-none focus:ring-4 focus:ring-brand-green/30 min-h-[180px] resize-none overflow-hidden text-center"
                />
                <textarea 
                  value={config.heroSubtitle}
                  onChange={(e) => updateHeroConfig({ heroSubtitle: e.target.value })}
                  className="w-full bg-white/5 border-none rounded-3xl p-6 text-xl md:text-2xl font-medium italic text-white/80 focus:outline-none focus:ring-4 focus:ring-brand-green/30 min-h-[120px] resize-none text-center"
                />
              </div>
            ) : (
              <div className="max-w-4xl">
                <h1 className="text-6xl md:text-9xl font-black mb-8 tracking-tighter leading-[0.85] uppercase italic animate-in fade-in slide-in-from-bottom duration-700">
                  {config.heroTitle}
                </h1>
                <p className="text-xl md:text-3xl font-medium mb-12 text-brand-light opacity-80 italic animate-in fade-in slide-in-from-bottom delay-200 duration-700">
                  {config.heroSubtitle}
                </p>
              </div>
            )}

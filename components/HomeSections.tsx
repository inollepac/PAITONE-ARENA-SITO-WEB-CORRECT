...
// Define the missing HomeSectionsProps interface
...
const ElementPositionEditor: React.FC<{
  style: SectionElement['style'];
  onUpdate: (updates: Partial<SectionElement['style']>) => void;
}> = ({ style, onUpdate }) => {
  const s = style || {};
  return (
    <div className="space-y-4 pt-4 border-t border-gray-100 text-left" onClick={(e) => e.stopPropagation()}>
...
const SectionEditorPanel: React.FC<{
  ...
  return (
    <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-brand-blue text-white px-6 py-3 rounded-full shadow-2xl z-[100]" onClick={(e) => e.stopPropagation()}>
      <button onClick={(e) => { e.stopPropagation(); onMove('up'); }} className="hover:text-brand-green p-1 transition-all active:scale-75"><i className="fas fa-arrow-up text-xs"></i></button>
      <button onClick={(e) => { e.stopPropagation(); onMove('down'); }} className="hover:text-brand-green p-1 transition-all active:scale-75"><i className="fas fa-arrow-down text-xs"></i></button>
      <div className="w-px h-4 bg-white/20 mx-2"></div>
      
      <button onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }} className={`p-1 hover:text-brand-green transition ${isOpen ? 'text-brand-green' : ''}`}>
        <i className="fas fa-paint-brush text-xs"></i>
      </button>

      <button onClick={(e) => { e.stopPropagation(); onAddElement('text'); }} className="p-2 hover:text-brand-green transition" title="Aggiungi Testo">
        <i className="fas fa-font text-xs"></i>
      </button>
      <button onClick={(e) => { e.stopPropagation(); onAddElement('image'); }} className="p-2 hover:text-brand-green transition" title="Aggiungi Immagine">
        <i className="fas fa-image text-xs"></i>
      </button>
      <button onClick={(e) => { e.stopPropagation(); onAddElement('logo'); }} className="p-2 hover:text-brand-green transition" title="Aggiungi Logo">
        <i className="fas fa-star text-xs"></i>
      </button>
      
      <div className="w-px h-4 bg-white/20 mx-2"></div>
      <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="hover:text-red-400 p-1"><i className="fas fa-trash text-xs"></i></button>

      {isOpen && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 bg-white text-brand-blue p-8 rounded-[3rem] shadow-2xl border border-brand-blue/10 w-96 animate-in fade-in zoom-in duration-200 z-[110]">
          ...
          {tab === 'layout' ? (
              ...
              <div className="pt-4 border-t border-gray-100 space-y-4">
                 ...
                 <div>
                    ...
                    <div className="flex items-center gap-3">
                      <input type="color" value={s.borderColor || '#A8D38E'} onChange={e => updateStyle('borderColor', e.target.value)} className="w-10 h-10 p-0 border-0 rounded-lg cursor-pointer bg-transparent" />
            ...
            <div className="space-y-6">
              {s.variant === 'image-bg' ? (
                <div className="space-y-4">
                  <button onClick={(e) => { e.stopPropagation(); onImageUpload(); }} className="w-full py-3 bg-brand-light text-brand-blue rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-green transition">Cambia Immagine Sfondo</button>
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <span className="text-[9px] font-black opacity-40 uppercase">Effetto Parallasse</span>
                    <button onClick={() => updateStyle('parallax', !s.parallax)} className={`w-12 h-6 rounded-full transition-all relative ${s.parallax ? 'bg-brand-green shadow-[0_0_10px_rgba(168,211,142,0.5)]' : 'bg-gray-300'}`}>
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1 no-scrollbar">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {GRADIENTS.map(g => (
                        <button key={g.name} onClick={() => updateStyle('bgGradient', g.value)} className={`p-3 rounded-xl text-[7px] font-black uppercase border transition-all ${s.bgGradient === g.value ? 'ring-2 ring-brand-blue shadow-lg scale-[1.02]' : 'border-gray-100 opacity-60 hover:opacity-100'}`} style={{ background: g.value || '#f3f4f6' }}>{g.name}</button>
                      ))}
                    </div>
                  </div>
                </div>
...
const LogoElementEditor: React.FC<{
  element: SectionElement;
  onUpdate: (content: string, style: Partial<SectionElement['style']>) => void;
  onDuplicate: () => void;
}> = ({ element, onUpdate, onDuplicate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [tab, setTab] = useState<'style' | 'position'>('style');
  const style = element.style || {};

  return (
    <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-brand-blue text-white px-3 py-1.5 rounded-full shadow-lg z-[100] animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
      <button onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }} className="text-[9px] font-black uppercase tracking-tighter active:scale-95"><i className="fas fa-cog mr-1"></i> Edit Logo</button>
      <button onClick={(e) => { e.stopPropagation(); onDuplicate(); }} className="text-[9px] font-black uppercase border-l border-white/20 pl-2 active:scale-95"><i className="fas fa-clone"></i></button>
      {isOpen && (
        <div className="absolute top-10 left-1/2 -translate-x-1/2 bg-white p-6 rounded-[2.5rem] shadow-2xl border border-brand-blue/20 w-80 z-[110] text-brand-blue text-left" onClick={(e) => e.stopPropagation()}>
          ...
          {tab === 'style' ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="range" min="20" max="600" value={parseInt(style.width || '120')} onChange={e => onUpdate(element.content, { width: `${e.target.value}px`, height: 'auto' })} className="w-full accent-brand-blue" />
              </div>
              <div>
                <label className="text-[8px] font-black opacity-40 uppercase mb-2 block">Effetto Ombra</label>
                <select value={Object.keys(BOX_SHADOWS).find(k => (BOX_SHADOWS as any)[k] === style.shadow) || 'none'} onChange={e => onUpdate(element.content, { shadow: (BOX_SHADOWS as any)[e.target.value] })} className="w-full bg-gray-50 p-2 rounded-lg text-[10px] font-bold outline-none border-0">
              ...
              <div>
                ...
                <input type="range" min="0" max="100" value={parseInt(style.borderRadius || '0')} onChange={e => onUpdate(element.content, { borderRadius: `${e.target.value}%` })} className="w-full accent-brand-blue" />
              </div>
            </div>
          ) : (
...
const TextElementEditor: React.FC<{
  ...
  onDuplicate: () => void;
}> = ({ element, onUpdateStyle, onDuplicate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [tab, setTab] = useState<'style' | 'position'>('style');
  const style = element.style || {};

  return (
    <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-brand-green text-brand-blue px-3 py-1.5 rounded-full shadow-lg z-[100] animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
      <button onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }} className="text-[9px] font-black uppercase tracking-tighter active:scale-95"><i className="fas fa-text-height mr-1"></i> Edit Text</button>
      <button onClick={(e) => { e.stopPropagation(); onDuplicate(); }} className="text-[9px] font-black uppercase border-l border-brand-blue/20 pl-2 active:scale-95"><i className="fas fa-clone"></i></button>
      {isOpen && (
        <div className="absolute top-10 left-1/2 -translate-x-1/2 bg-white p-6 rounded-[2.5rem] shadow-2xl border border-brand-green/20 w-80 z-[110] text-brand-blue text-left" onClick={(e) => e.stopPropagation()}>
          <div className="flex gap-2 mb-4 bg-gray-50 p-1 rounded-full">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(['left', 'center', 'right'] as const).map(align => (
                    <button key={align} onClick={() => onUpdateStyle({ textAlign: align })} className={`flex-1 p-3 rounded-xl transition-all ${style.textAlign === align ? 'bg-brand-blue text-white shadow-md scale-105' : 'bg-gray-50 text-brand-blue/40 border border-gray-100 hover:bg-gray-100'}`}>
...
const ImageElementEditor: React.FC<{
  ...
  onReplace: () => void;
  onDuplicate: () => void;
}> = ({ element, onUpdateStyle, onReplace, onDuplicate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [tab, setTab] = useState<'style' | 'position'>('style');
  const style = element.style || {};

  const resetFilters = () => {
    onUpdateStyle({ brightness: 1, contrast: 1, grayscale: 0, sepia: 0, blur: 0, hueRotate: 0, saturate: 1, invert: 0 });
  };

  return (
    <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-brand-blue text-white px-3 py-1.5 rounded-full shadow-lg z-[100] animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
      <button onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }} className="text-[9px] font-black uppercase tracking-tighter active:scale-95"><i className="fas fa-image mr-1"></i> Edit Image</button>
      <button onClick={(e) => { e.stopPropagation(); onDuplicate(); }} className="text-[9px] font-black uppercase border-l border-white/20 pl-2 active:scale-95"><i className="fas fa-clone"></i></button>
      {isOpen && (
        <div className="absolute top-10 left-1/2 -translate-x-1/2 bg-white p-6 rounded-[2.5rem] shadow-2xl border border-brand-blue/20 w-80 z-[110] text-brand-blue text-left" onClick={(e) => e.stopPropagation()}>
          <div className="flex gap-2 mb-4 bg-gray-50 p-1 rounded-full">
          ...
          {tab === 'style' ? (
            <div className="space-y-6 max-h-[400px] overflow-y-auto no-scrollbar pr-1">
              <button onClick={(e) => { e.stopPropagation(); onReplace(); }} className="w-full py-3 rounded-xl bg-brand-light text-brand-blue text-[9px] font-black uppercase tracking-widest hover:bg-brand-green transition">Cambia Immagine</button>
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                ...
                <select value={Object.keys(BOX_SHADOWS).find(k => (BOX_SHADOWS as any)[k] === style.shadow) || 'none'} onChange={e => onUpdateStyle({ shadow: (BOX_SHADOWS as any)[e.target.value] })} className="w-full bg-gray-50 p-3 rounded-xl text-[10px] font-bold outline-none border-0">
                  <option value="none">Nessuna</option>
                  <option value="soft">Morbida</option>
                  <option value="medium">Media</option>
                  <option value="heavy">Marcata</option>
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[{ id: 'none', label: 'Statica' }, { id: 'zoom', label: 'Zoom' }, { id: 'lift', label: 'Lift' }, { id: 'brighten', label: 'Glow' }].map(eff => (
                    <button key={eff.id} onClick={() => onUpdateStyle({ hoverEffect: eff.id as any })} className={`py-2 rounded-xl text-[8px] font-black uppercase transition-all ${style.hoverEffect === eff.id ? 'bg-brand-blue text-white shadow-md' : 'bg-gray-50 text-brand-blue/40 border border-gray-100'}`}>{eff.label}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['auto', '1/1', '4/3', '16/9', '9/16', '21/9'].map(ratio => (
                    <button key={ratio} onClick={() => onUpdateStyle({ aspectRatio: ratio })} className={`py-2 rounded-xl text-[8px] font-black uppercase transition-all ${style.aspectRatio === ratio ? 'bg-brand-blue text-white shadow-md' : 'bg-gray-50 text-brand-blue/40 border border-gray-100'}`}>{ratio}</button>
                  ))}
                </div>
              </div>
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <label className="text-[8px] font-black opacity-40 uppercase block mb-2">Ritaglio (Object Fit)</label>
                <select value={style.objectFit || 'cover'} onChange={e => onUpdateStyle({ objectFit: e.target.value as any })} className="w-full bg-gray-50 p-3 rounded-xl text-[10px] font-bold outline-none border-0">
              ...
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <label className="text-[8px] font-black opacity-40 uppercase block mb-2">Dimensioni Larghezza</label>
                <select value={style.width || '100%'} onChange={e => onUpdateStyle({ width: e.target.value })} className="w-full bg-gray-50 p-2 rounded-lg text-[10px] font-bold">
                  <option value="100%">Intera</option>
                  <option value="75%">Tre Quarti</option>
                  <option value="50%">Metà</option>
                  <option value="33%">Un Terzo</option>
                  <option value="400px">Fisso (400px)</option>
...
const HomeSections: React.FC<HomeSectionsProps> = ({ config, isEditMode, onUpdateConfig, onNavigate, events, courts }) => {
  ...
  const updateSection = (id: string, updates: Partial<SectionContent>, save = true) => {
    onUpdateConfig({ ...config, sections: config.sections.map(s => s.id === id ? { ...s, ...updates } : s) }, save);
  };
  ...
  const addElement = (sectionId: string, type: 'text' | 'image' | 'logo') => {
    const newElement: SectionElement = {
      ...
      style: { width: type === 'logo' ? '120px' : '100%', height: 'auto', borderRadius: type === 'logo' ? '0%' : '3rem', fontSize: '1.125rem', color: config.primaryColor, textAlign: 'center', textShadow: 'none', fontWeight: 'normal', lineHeight: '1.5', x: 0, y: 0, scale: 1, rotation: 0, zIndex: 5, brightness: 1, contrast: 1, grayscale: 0, sepia: 0, blur: 0, hueRotate: 0, saturate: 1, invert: 0, borderWidth: 0, borderColor: config.accentColor, hoverEffect: 'none', aspectRatio: 'auto', objectFit: 'cover' }
    };
    onUpdateConfig({ ...config, sections: config.sections.map(s => s.id === sectionId ? { ...s, elements: [...(s.elements || []), newElement] } : s) });
  };

  const updateElement = (sectionId: string, elId: string, content: string, save = true) => {
    onUpdateConfig({ ...config, sections: config.sections.map(s => s.id === sectionId ? { ...s, elements: s.elements?.map(el => el.id === elId ? { ...el, content } : el) } : s) }, save);
  };

  const updateElementStyle = (sectionId: string, elId: string, styleUpdates: Partial<SectionElement['style']>) => {
    onUpdateConfig({ ...config, sections: config.sections.map(s => s.id === sectionId ? { ...s, elements: s.elements?.map(el => el.id === elId ? { ...el, style: { ...(el.style || {}), ...styleUpdates } } : el) } : s) });
  };
  ...
  const duplicateElement = (sectionId: string, element: SectionElement) => {
    ...
    onUpdateConfig({ ...config, sections: config.sections.map(s => s.id === sectionId ? { ...s, elements: [...(s.elements || []), duplicate] } : s) });
  };
  ...
  return (
    <div className="space-y-24 pb-48 overflow-x-hidden">
      ...
      {config.sections.map((section, idx) => {
        ...
        const containerStyle: React.CSSProperties = { borderWidth: `${s.borderWidth}px`, borderColor: s.borderColor, borderStyle: s.borderWidth > 0 ? 'solid' : 'none', backgroundColor: (s.variant === 'solid' || s.variant === 'custom') ? s.bgColor : undefined, backgroundImage: s.variant === 'image-bg' ? `url(${s.bgImageUrl})` : (s.bgGradient || 'none'), backgroundAttachment: s.parallax ? 'fixed' : 'scroll', backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' };
        const widthClass = s.width === 'full' ? 'w-full' : s.width === 'narrow' ? 'max-w-4xl' : 'max-w-7xl';
        return (
          <div key={section.id} style={containerStyle} className={`relative transition-all duration-700 mx-auto ${STYLE_MAPS.variant[s.variant || 'solid']} ${STYLE_MAPS.shape[s.shape || 'rounded']} ${STYLE_MAPS.padding[s.padding || 'medium']} ${STYLE_MAPS.shadow[s.shadow || 'none']} ${isEditMode ? 'ring-2 ring-dashed ring-brand-blue/10 m-6' : ''}`}>
            {s.variant === 'image-bg' && s.bgOpacity !== undefined && <div className="absolute inset-0 pointer-events-none transition-all duration-500" style={{ backgroundColor: `rgba(0,0,0,${1 - (s.bgOpacity || 1)})` }}></div>}
            {isEditMode && <SectionEditorPanel section={section} onUpdate={(upd) => updateSection(section.id, upd)} onDelete={() => onUpdateConfig({ ...config, sections: config.sections.filter(sec => sec.id !== section.id) })} onMove={(dir) => moveSection(idx, dir)} onAddElement={(type) => addElement(section.id, type)} onImageUpload={() => handleImageClick(section.id, undefined, true)} />}
            <div className={`${widthClass} mx-auto px-6 relative z-10`}>
              <div className="text-center space-y-6">
                {isEditMode ? (
                  <div className="space-y-4">
                    <input value={section.title} onChange={e => updateSection(section.id, { title: e.target.value }, false)} className="text-4xl md:text-7xl font-black uppercase text-center w-full bg-transparent outline-none italic tracking-tighter" />
                    <textarea value={section.description} onChange={e => updateSection(section.id, { description: e.target.value }, false)} className="text-xl opacity-60 italic text-center w-full bg-transparent outline-none resize-none min-h-[100px]" />
                  </div>
                ) : (
                ...
                <div className="flex flex-wrap items-center justify-center gap-12 mt-20 relative min-h-[100px]">
                  {section.elements?.map(el => {
                    ...
                    const transformStyle: React.CSSProperties = { transform: `translate(${elStyle.x || 0}px, ${elStyle.y || 0}px) rotate(${elStyle.rotation || 0}deg) scale(${elStyle.scale || 1})`, zIndex: elStyle.zIndex || 5, width: elStyle.width || '100%', position: 'relative', transition: 'transform 0.2s ease-out' };
                    const removeControl = isEditMode && (
                      <div className="absolute -top-4 -right-4 flex gap-2 z-[100] opacity-0 group-hover/el:opacity-100 transition-all">
                        <button onClick={(e) => { e.stopPropagation(); onUpdateConfig({ ...config, sections: config.sections.map(sec => sec.id === section.id ? { ...sec, elements: sec.elements?.filter(e => e.id !== el.id) } : sec) }); }} className="w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center shadow-xl hover:scale-110 active:scale-90"><i className="fas fa-times text-xs"></i></button>
                      </div>
                    );
                    if (el.type === 'text') return (
                      <div key={el.id} style={transformStyle} className="group/el">
                        ...
                        <div className="relative p-8 rounded-[3rem] bg-white/5 border border-white/10 backdrop-blur-sm shadow-inner flex flex-col items-center justify-center" style={{ textAlign: elStyle.textAlign as any, color: elStyle.color, fontSize: elStyle.fontSize, fontWeight: elStyle.fontWeight, lineHeight: elStyle.lineHeight, textShadow: elStyle.textShadow }}>
                          ...
                          {isEditMode ? <textarea value={el.content} onChange={e => updateElement(section.id, el.id, e.target.value, false)} className="w-full bg-transparent outline-none italic opacity-80 resize-none h-32 text-center" /> : <p className="italic opacity-80 whitespace-pre-wrap">{el.content}</p>}
                        </div>
                      </div>
                    );
                    if (el.type === 'image') return (
                      <div key={el.id} style={transformStyle} className="group/el" onClick={(e) => { e.stopPropagation(); isEditMode && handleImageClick(section.id, el.id); }}>
                        {removeControl}
                        <div style={{ borderRadius: elStyle.borderRadius || '3rem', height: elStyle.height || 'auto', minHeight: '200px', overflow: 'hidden', boxShadow: elStyle.shadow || 'none', aspectRatio: elStyle.aspectRatio !== 'auto' ? elStyle.aspectRatio : undefined }} className={`relative cursor-pointer transition-all duration-700 ease-out ${STYLE_MAPS.shadow[elStyle.hoverEffect as any] || ''}`}>
                          ...
                          <img src={el.content} style={{ filter: `brightness(${elStyle.brightness || 1}) contrast(${elStyle.contrast || 1}) grayscale(${elStyle.grayscale || 0}) sepia(${elStyle.sepia || 0}) blur(${elStyle.blur || 0}px) hue-rotate(${elStyle.hueRotate || 0}deg) saturate(${elStyle.saturate || 1}) invert(${elStyle.invert || 0})`, objectFit: elStyle.objectFit || 'cover', width: '100%', height: '100%' }} className="transition-transform duration-1000" alt="Arena Design Asset" />
                        </div>
                      </div>
                    );
                    if (el.type === 'logo') return (
                      <div key={el.id} style={transformStyle} className="group/el">
                        {removeControl}
                        {isEditMode && <LogoElementEditor element={el} onDuplicate={() => duplicateElement(section.id, el)} onUpdate={(content, styleUpdates) => updateSection(section.id, { elements: section.elements?.map(e => e.id === el.id ? { ...e, content, style: { ...e.style, ...styleUpdates } } : e) })} />}
                        <div style={{ width: elStyle.width || '120px', borderRadius: elStyle.borderRadius || '0%', boxShadow: elStyle.shadow || 'none' }} className="bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl flex items-center justify-center p-4 overflow-hidden">
                          <img src={el.content === 'primary' ? config.primaryLogoUrl : config.secondaryLogoUrl || 'https://via.placeholder.com/150'} className="w-full h-full object-contain" alt="Logo Asset" />
                        </div>
                      </div>
                    );
                    return null;
      ...
      {isEditMode && <div className="flex justify-center pt-24"><button onClick={() => onUpdateConfig({ ...config, sections: [...config.sections, { id: `custom_${Date.now()}`, title: 'Nuova Area Progetto', navLabel: 'New Area', enabled: true, isCustom: true, elements: [], style: { variant: 'solid', shape: 'rounded', padding: 'medium', width: 'contained', shadow: 'soft', borderWidth: 0, borderColor: config.accentColor, bgColor: '#FFFFFF', bgGradient: '', parallax: false } }] })} className="group flex flex-col items-center gap-6 bg-white border-4 border-dashed border-brand-blue/10 px-24 py-16 rounded-[6rem] text-brand-blue/30 hover:text-brand-green hover:border-brand-green transition-all shadow-xl"><div className="w-24 h-24 bg-brand-light rounded-full flex items-center justify-center text-5xl group-hover:bg-brand-green group-hover:text-brand-blue transition-all"><i className="fas fa-plus"></i></div><div className="text-center"><span className="block text-3xl font-black uppercase italic tracking-tighter">Nuovo Spazio Arena</span><span className="text-[10px] uppercase font-bold tracking-[0.3em] opacity-60">Aggiungi una sezione da modellare</span></div></button></div>}
    </div>
  );
};

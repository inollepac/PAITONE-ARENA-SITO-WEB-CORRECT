...
const HomeSections: React.FC<HomeSectionsProps> = ({ config, isEditMode, onUpdateConfig }) => {
  ...
  const [elementEditorTab, setElementEditorTab] = useState<'content' | 'style' | 'filters' | 'layout'>('content');
  const [activeSectionEditor, setActiveSectionEditor] = useState<string | null>(null);
  const [sectionEditorTab, setSectionEditorTab] = useState<'background' | 'style' | 'layout'>('background');
        ...
        {isEditMode && activeEditor && activeElement && (
          ...
          >
            ...
            <div className="space-y-6">
              {/* Tabs */}
              <div className="flex bg-gray-50 rounded-2xl p-1 mb-4">
                {[
                  { id: 'content', icon: 'fa-edit', label: 'Contenuto' },
                  { id: 'style', icon: 'fa-paint-brush', label: 'Stile' },
                  { id: 'filters', icon: 'fa-magic', label: 'Filtri' },
                  { id: 'layout', icon: 'fa-layer-group', label: 'Layout' }
                ].map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => setElementEditorTab(tab.id as any)}
                    className={`flex-1 py-2 rounded-xl flex flex-col items-center gap-1 transition-all ${elementEditorTab === tab.id ? 'bg-brand-blue text-white shadow-md' : 'text-brand-blue/30 hover:bg-white'}`}
                  >
              ...
              {elementEditorTab === 'content' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="flex gap-2">
                    <button 
                      onClick={() => duplicateElement(activeEditor.sId, activeElement)}
                      className="flex-1 py-3 bg-brand-green/10 text-brand-green rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-brand-green hover:text-brand-blue transition-all"
                    >
                  ...
                  {activeElement.type === 'text' && (
                    <div className="space-y-2">
                      <label className="text-[8px] font-bold uppercase block mb-2 text-left">Testo</label>
                      <textarea 
                        value={activeElement.content || ''} 
                        onChange={e => updateElement(activeEditor.sId, activeElement.id, { content: e.target.value })}
                        className="w-full bg-gray-50 p-3 rounded-xl text-[10px] font-black border-none outline-none h-32 resize-none"
                      />
                    </div>
                  )}
              ...
              {elementEditorTab === 'style' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  {activeElement.type === 'text' ? (
                    ...
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase text-brand-blue opacity-30 text-left">Stile Immagine</h4>
              ...
              {elementEditorTab === 'filters' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <h4 className="text-[9px] font-black uppercase text-brand-blue opacity-30 text-left">Filtri ed Effetti</h4>
                    ...
                    {[
                      ...
                      <div key={f.key}>
                        ...
                        <input 
                          ...
                          value={(activeElement.style as any)[f.key] ?? (['brightness', 'contrast', 'saturate'].includes(f.key) ? 1 : 0)} 
              ...
              {elementEditorTab === 'layout' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <h4 className="text-[9px] font-black uppercase text-brand-blue opacity-30 text-left">Posizionamento e Layer</h4>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[8px] font-bold uppercase block mb-2 text-left">Offset X ({activeElement.style?.x || 0}px)</label>
                      <input type="range" min="-500" max="500" step="1" value={activeElement.style?.x || 0} onChange={e => updateElementStyle(activeEditor.sId, activeEditor.elId, { x: +e.target.value })} className="w-full accent-brand-blue" />
                    </div>
                    <div>
                      <label className="text-[8px] font-bold uppercase block mb-2 text-left">Offset Y ({activeElement.style?.y || 0}px)</label>
                      <input type="range" min="-500" max="500" step="1" value={activeElement.style?.y || 0} onChange={e => updateElementStyle(activeEditor.sId, activeEditor.elId, { y: +e.target.value })} className="w-full accent-brand-blue" />
                    </div>
                  </div>

                  <div>
                    <label className="text-[8px] font-bold uppercase block mb-2 text-left">Scala Totale ({activeElement.style?.scale || 1})</label>
                    <input type="range" min="0.1" max="4" step="0.1" value={activeElement.style?.scale || 1} onChange={e => updateElementStyle(activeEditor.sId, activeEditor.elId, { scale: +e.target.value })} className="w-full accent-brand-green" />
                  </div>

                  <div>
                    <label className="text-[8px] font-bold uppercase block mb-2 text-left">Ordine Livello (Z-Index: {activeElement.style?.zIndex || 5})</label>
                    <input type="range" min="1" max="100" step="1" value={activeElement.style?.zIndex || 5} onChange={e => updateElementStyle(activeEditor.sId, activeEditor.elId, { zIndex: +e.target.value })} className="w-full accent-brand-blue" />
                  </div>

                  {activeElement.type !== 'text' && (
                    <div>
                      <label className="text-[8px] font-bold uppercase block mb-2 text-left">Larghezza ({activeElement.style?.width || '320px'})</label>
                      <input 
                        type="text" 
                        value={activeElement.style?.width || '320px'} 
                        onChange={e => updateElementStyle(activeEditor.sId, activeEditor.elId, { width: e.target.value })}
                        className="w-full bg-gray-50 p-2 rounded-xl text-[10px] font-black border-none outline-none"
                        placeholder="es: 320px o 50%"
                      />
                    </div>
                  )}
                </div>
        ...
        {isEditMode && activeSectionEditor && activeSection && (
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="flex bg-gray-50 rounded-2xl p-1 mb-4">
                {[
                  { id: 'background', icon: 'fa-image', label: 'Sfondo' },
                  { id: 'style', icon: 'fa-paint-brush', label: 'Bordi' },
                  { id: 'layout', icon: 'fa-layer-group', label: 'Layout' }
                ].map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => setSectionEditorTab(tab.id as any)}
                    className={`flex-1 py-2 rounded-xl flex flex-col items-center gap-1 transition-all ${sectionEditorTab === tab.id ? 'bg-brand-blue text-white shadow-md' : 'text-brand-blue/30 hover:bg-white'}`}
                  >
                    <i className={`fas ${tab.icon} text-[10px]`}></i>
                    <span className="text-[7px] font-black uppercase tracking-tighter">{tab.label}</span>
                  </button>
              ...
              {sectionEditorTab === 'style' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-left-2 duration-300">
                  ...
                  <div>
                    <label className="text-[8px] font-bold uppercase block mb-2 text-left">Ombra Sezione</label>
                    <select 
                      value={activeSection.style?.shadow || 'none'} 
                      onChange={e => updateSectionStyle(activeSection.id, { shadow: e.target.value as any })}
                      className="w-full bg-gray-50 p-3 rounded-xl text-[10px] font-black border-none outline-none"
                    >
                      <option value="none">Nessuna</option>
                      <option value="soft">Morbida</option>
              ...
              {sectionEditorTab === 'layout' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-left-2 duration-300">
                  <h4 className="text-[9px] font-black uppercase text-brand-blue opacity-30 text-left">Layout e Forma</h4>
                  <div>
                    <label className="text-[8px] font-bold uppercase block mb-2 text-left">Forma Sezione</label>
                    <select 
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    <button 
                      ...
                      className="w-full py-3 bg-brand-green/10 text-brand-green rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-brand-green hover:text-brand-blue transition-all"
                    >
                      <i className="fas fa-copy mr-2"></i> Duplica Sezione
                    </button>
                  </div>
                </div>
              )}
